...
bibliography: 2024-12-07-scRNAseq-assumptions.bib
...
Since a cell's gene expression profile at a given point in time can be thought of as a measurement of the cell's current "state",
scRNA-seq data is also particularly appealing in the context of understanding how external stimuli affect cell state. Techniques such as Perturb-seq <d-cite key="dixitPerturbseqDissectingMolecular2016"></d-cite> leverage scRNA-seq as a way to generate high-throughput measurements of how individual stimuli affect cells. Using such data to gain a better understanding of the intricate
genetic circuitry within our cells has led to a burgeoning field of "perturbation modeling". By understanding how a given perturbation affects our cells, researchers hope to gain insight into the genetic mechanisms of various diseases as well as
understand how specific therapeutics could be used to perturb a cell out of an unhealthy state and into a healthy one.
...
In parallel to the explosion of available scRNA-seq data, the machine learning field has seen a growth of increasingly powerful generative modeling techniques.
Generative models are typically deep learning models that learn smooth representations of the underlying distribution from which their training data is sampled. These models of real data can then be sampled from to generate novel, highly realistic
generated data. While the ability to create realistic datapoints alone has been exciting, the ability to *condition* generations
with some user-specified criteria has led to numerous applications in a broad range of fields including generating images that
match a given caption <d-cite key="esserScalingRectifiedFlow2024"></d-cite>, generating music of a given style <d-cite key="copetSimpleControllableMusic2024"></d-cite>, and even proteins
that enact a specified function <d-cite key="ingrahamIlluminatingProteinSpace2023"></d-cite>.
...
Recent work has focused on developing scRNA-seq foundation models as an approach to solve the challenge of integrating diverse sets of scRNA-seq datasets in a scalable and generalizable way <d-cite key="theodorisTransferLearningEnables2023"></d-cite> <d-cite key="yangScBERTLargescalePretrained2022"></d-cite> <d-cite key="cuiScGPTBuildingFoundation2023"></d-cite> <d-cite key="chenGeneptSimpleHardtoBeat2023"></d-cite> <d-cite key="yangGeneCompassDecipheringUniversal2023"></d-cite> <d-cite key="haoLargeScaleFoundation2023"></d-cite> <d-cite key="levineCell2SentenceTeachingLarge2023"></d-cite>.
While some of these models are directly used for perturbation modeling, other models build on top of the representations produced by a foundation model to perform generation <d-cite key="luoScDiffusionConditionalGeneration2024"></d-cite> through a process called latent diffusion.

In this post, we'll explore a fundamental assumption of three such foundation models (Geneformer<d-cite key="theodorisTransferLearningEnables2023"></d-cite>, cell2sentence <d-cite key="levineCell2SentenceTeachingLarge2023"></d-cite>, and GenePT <d-cite key="chenGeneptSimpleHardtoBeat2023"></d-cite>), which is the assertion that a given gene expression profile can be well-approximated by a rank-value encoding of genes.
